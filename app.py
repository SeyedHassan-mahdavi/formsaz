...
# ---- MUST be first (Windows asyncio policy + env) ----
import sys, asyncio, os, pathlib, json, re
from dotenv import load_dotenv
if sys.platform.startswith("win"):
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
load_dotenv()
...
# --- Flows / Routers / Utils ---
from flows.common import cancel_cmd
from flows.units import (
    build_unit_wizard_conversation,
    build_unit_attach_conversation,
    build_unit_admins_manager_conversation,
    unit_add_cmd, unit_list_cmd, unit_attach_cmd,
    build_unit_list_conversation, ul_start
)
from flows.report import build_report_conversation
from flows.manage import (
    campaigns_cmd as manage_campaigns_cmd, manage_cb, edit_text_receiver,
    edit_platforms_toggle, campaigns_search_receiver, campaigns_browse_cb
)
from flows.admin import addadmin_cmd, linkadmin_cmd, myusers_cmd, adduser_cmd, renameuser_cmd, deluser_cmd
from flows.user_shortcuts import mystats_cmd, myexport_cmd, user_cb
from flows.newcampaign import newcampaign, build_conversation as build_newcamp_conversation
...
from database import init_db, SessionLocal
from models import Admin
from keyboards import user_reply_kb, admin_reply_kb, superadmin_reply_kb, BTN_SA_DASH
from crud import is_admin, is_superadmin, get_user_admin, list_campaigns_for_admin_units
from flows.superadmin import dashboard_entry, sa_router, adm_router
from re import escape as re_escape
...
# --- PTB startup hook: runs inside PTB's own event loop ---
async def on_startup(app: Application):
    hard_admins = parse_int_set_env("HARD_ADMINS")
    await init_db()
    await bootstrap_admins(hard_admins)
...
def main():
    ...
    app: Application = ApplicationBuilder().token(token).post_init(on_startup).build()

    # 1) گزارش (Conversation) – قبل از بقیه
    app.add_handler(build_report_conversation())

    # 2) ویرایش‌های درون فرم‌ها (non-blocking)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, edit_text_receiver, block=False), group=-1)
    ...
    # 5) واحدها — CLI
    app.add_handler(CommandHandler("unit_add", unit_add_cmd))
    app.add_handler(CommandHandler("unit_list", unit_list_cmd))
    app.add_handler(CommandHandler("unit_attach", unit_attach_cmd))

    # 6) واحدها — ویزارد/لیست
    app.add_handler(build_unit_wizard_conversation())
    app.add_handler(build_unit_attach_conversation())
    app.add_handler(build_unit_list_conversation())
    app.add_handler(build_unit_admins_manager_conversation())

    # 7) کمپین‌ها
    app.add_handler(CommandHandler("campaigns", manage_campaigns_cmd))
    app.add_handler(CallbackQueryHandler(edit_platforms_toggle, pattern=r"^epf:"))
    app.add_handler(CallbackQueryHandler(
        manage_cb,
        pattern=r"^(camp:\d+:manage|edit:\d+:[a-z_]+|(?:toggle|delete|delok|stats|export):\d+)$"
